...
void CalcRadiance(PS_INPUT input, float3 viewDir, float3 normalVec, float3 lightPos, float3 lightCol, float3 F0, out float3 rad)
{
	...
	float NdotL = max(dot(normalVec, lightDir), 0.0f);
	rad = (((kD * objectMaterial.albedo / PI) + specular) * radiance * NdotL);
}
...
float4 main(PS_INPUT input) : SV_TARGET
{
	...
	float3 albedotex = albedoSRV.Sample(basicSampler, input.uv).rgb; //pow(, 2.2f)
	...
	F0 = lerp(F0, objectMaterial.albedo, cbmetallic);
	...
	float3 diffuse = objectMaterial.albedo * irradiance;
